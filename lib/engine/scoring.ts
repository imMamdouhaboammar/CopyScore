...
const DOMAIN_IDS: DomainId[] = ['conversion_copywriting', 'content_creation', 'performance_copy', 'cro'];
...
export function calculateFinalScore(
  ...
  responses.forEach((r) => {
    if (domainResponses[r.domain]) domainResponses[r.domain].push(r);
  });
  ...
  const rawOverall = Math.round(domainValues.reduce((sum, domain) => sum + domain.scaledScore, 0) / domainValues.length);
  ...
  responses.forEach((r) => {
    ...
    if (r.isCorrect && q.diagnosticInsight.goodOutcome) {
      if (whatYouDidWell.length < 3 && !whatYouDidWell.includes(q.diagnosticInsight.goodOutcome)) whatYouDidWell.push(q.diagnosticInsight.goodOutcome);
    } else if (!r.isCorrect && q.diagnosticInsight.pitfall) {
      if (whatCostYouPoints.length < 3 && !whatCostYouPoints.includes(q.diagnosticInsight.pitfall)) whatCostYouPoints.push(q.diagnosticInsight.pitfall);
    }
  });

  if (whatYouDidWell.length === 0) whatYouDidWell.push('Demonstrated solid foundational awareness and audience alignment principles.');
  if (whatCostYouPoints.length === 0) whatCostYouPoints.push('Near flawless run; minimal deductions across tested scenario parameters.');
...
function computeDomainScore(domain: DomainId, responses: EvaluatedResponse[]): DomainScore {
  if (responses.length === 0) {
    return { domain, rawScore: 0, scaledScore: 50, questionsAttempted: 0, accuracy: 0, highestDifficultyCleared: 1, statusLabel: 'Competitive' };
  }
  let totalWeight = 0;
  let earnedWeight = 0;
  let correctCount = 0;
  ...
  const scaledScore = Math.max(15, Math.min(99, Math.round(30 + ratio * 68)));
  ...
  else if (scaledScore < 60) statusLabel = 'Needs Work';
  return { domain, rawScore: Math.round(earnedWeight * 10) / 10, scaledScore, questionsAttempted: responses.length, accuracy: Math.round(accuracy), highestDifficultyCleared: maxDiff, statusLabel };
}
